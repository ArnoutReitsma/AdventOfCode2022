...
foreach (string line in System.IO.File.ReadLines(@"..\..\..\input.txt"))
{
    ...
    if (lineNumb % 3 == 0)
    {
        var matchingItems2 = IntersertCharsInStringList(threePack);
...
List<char> IntersertCharsInStringList(List<string> list)
{
    return list.Select(s => s.ToList()).Aggregate<IEnumerable<char>>(
       (previousList, nextList) => previousList.Intersect(nextList)).ToList();
}